...
if __name__ == '__main__':
    ...
    for fold in folds:
        ...
        df.loc[validation_mask, 'oof_risk_score'] = model.predict_partial_hazard(df.loc[validation_mask, features])
        df.loc[validation_mask, 'oof_survival_probability'] = model.predict_survival_function(
        ...
        validation_scores = metrics.score(
            ...
            event_column='efs',
            prediction_column='oof_risk_score'
        )
    ...
    oof_mask = df['oof_risk_score'].notna()
    ...
    with open(model_directory / model_file_name, mode='wb') as f:
        pickle.dump(model, f)
    settings.logger.info(f'{model_file_name} is saved to {model_directory}')
    df.loc[:, 'risk_score'] = model.predict_partial_hazard(df.loc[:, features])
    df.loc[:, 'survival_probability'] = model.predict_survival_function(
        df.loc[:, features],
